...
@Service("SPtnMemcachedClient")
public class SPtnMemcachedClient {
	 ...
	 * @return
	 */
	public MemcachedInfoWrapper getCurrentMemcachedInfo() {
		MemcachedInfoWrapper memcachedInfoWrapper = new MemcachedInfoWrapper();
		try {
			MemcachedClient client = getCurrentMemcachedClient();
			Integer currentVersion = (Integer) client.get(CATEGORY_CURRENT_VERSION);
			Map<SocketAddress, Map<String, String>> map = client.getStats();
			Set<SocketAddress> socketAddressSet = map.keySet();
			if(socketAddressSet != null && socketAddressSet.size() > 0){
				List<MemcachedInfo> memcachedList = new ArrayList<MemcachedInfo>();
				for(SocketAddress address : socketAddressSet){
					MemcachedInfo memcachedInfo = new MemcachedInfo();
					memcachedInfo.setAddress(address.toString());
					Map<String, String> infoMap = map.get(address);
					memcachedInfo.setBytes(infoMap.get("bytes"));
					memcachedInfo.setVersion(infoMap.get("version"));
					memcachedInfo.setTime(infoMap.get("time"));
					memcachedInfo.setThreads(infoMap.get("threads"));
					memcachedInfo.setLimitMaxBytes(infoMap.get("limit_maxbytes"));
					memcachedInfo.setBytesRead(infoMap.get("bytes_read"));
					memcachedInfo.setBytesWritten(infoMap.get("bytes_written"));
					memcachedInfo.setCurrConnections(infoMap.get("curr_connections"));
					memcachedList.add(memcachedInfo);
				}
				memcachedInfoWrapper.setMemcachedList(memcachedList);
			}
			memcachedInfoWrapper.setCurrentVersion(currentVersion == null ? "" : currentVersion + "");
			if(currentVersion != null){
				String usabledVersion = null;
				List<Long> topCategoryIdList = (List<Long> ) get(client, "key", currentVersion);
				if(topCategoryIdList != null){
					usabledVersion = currentVersion + ""; 
				}
				Integer previousVersion = getPreviousVersion(currentVersion);
				topCategoryIdList = (List<Long> ) get(client, "key", previousVersion);
				if(topCategoryIdList != null){
					usabledVersion = usabledVersion + "," + previousVersion; 
				}
				...
				memcachedInfoWrapper.setUsabledVersion(usabledVersion);
			}
		} catch (Exception e) {
		}
		return memcachedInfoWrapper;
	}
